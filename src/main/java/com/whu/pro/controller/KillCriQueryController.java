...
@Controller
@RequestMapping("KillCriQueryController")
public class KillCriQueryController {
    private int type_id = 6002000;
    @Resource
    private KillCriQueryService KillCriqueryService;
    @Resource
    ...
    public void InsertElement(HttpServletRequest request, HttpServletResponse response) throws Exception { //根据要素id查询要素表获得要素信息返回前台显示
        KillCriQueryParam kcqp = new KillCriQueryParam();
        ...
        KillCriqueryService.InsertElement(kcqp);
        if (case_id != "") {
            System.out.println("---------------- 开始插入关联表");
            ArrayList<KillCriQueryResult> list = KillCriqueryService.selectMaxId();
            int element_id;
            if(list.size()==0) {
                element_id = 1;
            }else {
    ...
    public void UpdateElement(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        kcqp.setElement_id(Integer.parseInt(element_id));
        kcqp.setManager(manager);
        kcqp.setRemark(remark);
        kcqp.setElement_image(temp);
        kcqp.setBegin_date(create_date);
        kcqp.setLocalname(localname);
        KillCriqueryService.UpdateElement(kcqp);
    }
    ...
    public Object selectAllElement(String begin_date, String end_date, String key_word) {
        ...
        KillCriQueryParam kcqp = new KillCriQueryParam();
        kcqp.setKey_word(key_word);
        kcqp.setBegin_date(begin_date);
        kcqp.setEnd_date(end_date);
        ArrayList<KillCriQueryResult> list = KillCriqueryService.selectAllElement(kcqp);
    ...
    public void GetElementInfo(HttpServletRequest request, HttpServletResponse response) throws IOException { //根据要素id查询要素表获得要素信息返回前台显示
        ...
        kcqp.setElement_id(element_id);
        ArrayList<KillCriQueryResult> list = KillCriqueryService.GetElementInfo(kcqp);
    ...
    public void DeleteElement(String element_id, String type_id) throws Exception {
        ...
        KillCriqueryService.DeleteElement(kcqp);
