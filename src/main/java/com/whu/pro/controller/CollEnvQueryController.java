...
@Controller
@RequestMapping("CollEnvQueryController")
public class CollEnvQueryController {
    private int type_id = 7004000;
    @Resource
    private CollEnvQueryService CollEnvqueryService;
    @Resource
    ...
    public void InsertElement(HttpServletRequest request, HttpServletResponse response) throws Exception { //根据要素id查询要素表获得要素信息返回前台显示
        CollEnvQueryParam ceqp = new CollEnvQueryParam();
        ...
        ceqp.setBegin_date(create_date);
        ceqp.setElement_image(element_image);
        ceqp.setManager(manager);
        ceqp.setRemark(remark);
        ceqp.setLocalname(localname);
        CollEnvqueryService.InsertElement(ceqp);
        if (case_id != "") {
            System.out.println("---------------- 开始插入关联表");
            ArrayList<CollEnvQueryResult> list = CollEnvqueryService.selectMaxId();
            int element_id;
            if(list.size()==0) {
                element_id = 1;
            }else {
    ...
    public void UpdateElement(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        ceqp.setElement_id(Integer.parseInt(element_id));
        ceqp.setManager(manager);
        ceqp.setRemark(remark);
        ceqp.setElement_image(temp);
        ceqp.setBegin_date(create_date);
        ceqp.setLocalname(localname);
        CollEnvqueryService.UpdateElement(ceqp);
    }
    ...
    public Object selectAllElement(String begin_date, String end_date, String key_word) {
        ...
        CollEnvQueryParam ceqp = new CollEnvQueryParam();
        ceqp.setKey_word(key_word);
        ceqp.setBegin_date(begin_date);
        ceqp.setEnd_date(end_date);
        ArrayList<CollEnvQueryResult> list = CollEnvqueryService.selectAllElement(ceqp);
    ...
    public void GetElementInfo(HttpServletRequest request, HttpServletResponse response) throws IOException { //根据要素id查询要素表获得要素信息返回前台显示
        ...
        ceqp.setElement_id(element_id);
        ArrayList<CollEnvQueryResult> list = CollEnvqueryService.GetElementInfo(ceqp);
    ...
    public void DeleteElement(String element_id, String type_id) throws Exception {
        ...
        CollEnvqueryService.DeleteElement(ceqp);
