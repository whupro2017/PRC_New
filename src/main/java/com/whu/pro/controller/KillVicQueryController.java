...
@Controller
@RequestMapping("KillVicQueryController")
public class KillVicQueryController {
    private int type_id = 6001000;
    @Resource
    private KillVicQueryService KillVicqueryService;
    @Resource
    ...
    public void InsertElement(HttpServletRequest request, HttpServletResponse response) throws Exception { //根据要素id查询要素表获得要素信息返回前台显示
        KillVicQueryParam kvqp = new KillVicQueryParam();
        ...
        kvqp.setBegin_date(create_date);
        kvqp.setElement_image(element_image);
        kvqp.setManager(manager);
        kvqp.setRemark(remark);
        kvqp.setLocalname(localname);
        KillVicqueryService.InsertElement(kvqp);
        if (case_id != "") {
            System.out.println("---------------- 开始插入关联表");
            ArrayList<KillVicQueryResult> list = KillVicqueryService.selectMaxId();
            int element_id;
            if(list.size()==0) {
                element_id = 1;
            }else {
    ...
    public void UpdateElement(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        kvqp.setElement_id(Integer.parseInt(element_id));
        kvqp.setManager(manager);
        kvqp.setRemark(remark);
        kvqp.setElement_image(temp);
        kvqp.setBegin_date(create_date);
        kvqp.setLocalname(localname);
        KillVicqueryService.UpdateElement(kvqp);
    }
    ...
    public Object selectAllElement(String begin_date, String end_date, String key_word) {
        ...
        KillVicQueryParam kvqp = new KillVicQueryParam();
        kvqp.setKey_word(key_word);
        kvqp.setBegin_date(begin_date);
        kvqp.setEnd_date(end_date);
        ArrayList<KillVicQueryResult> list = KillVicqueryService.selectAllElement(kvqp);
    ...
    public void GetElementInfo(HttpServletRequest request, HttpServletResponse response) throws IOException { //根据要素id查询要素表获得要素信息返回前台显示
        ...
        kvqp.setElement_id(element_id);
        ArrayList<KillVicQueryResult> list = KillVicqueryService.GetElementInfo(kvqp);
    ...
    public void DeleteElement(String element_id, String type_id) throws Exception {
        ...
        KillVicqueryService.DeleteElement(kvqp);
